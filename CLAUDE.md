...
- **Veritabanı:** Hostinger MySQL — **BAĞLI**. Sunucu: `srv1764.hstgr.io`, DB/kullanıcı: `u851420727_ParselBul`.
  ...
  - **Yol haritası (kullanıcı onaylı, sırayla):**
    ...
    4. İmar durumu araştırması (belediye/e-imar — TKGM'de yok) ← SIRADAKİ
    5. PDF indirme + paylaşılabilir link
    6. (İsteğe bağlı) tüm-parsel kadastro overlay'i için resmî TKGM tile/WMS erişimi araştır
...
- Hostinger eski JS/CSS'i önbellekte tutabiliyor → deploy sonrası "stale asset" hatası (ör. eski main.js'in
  ...
  - `index.html` CSS/JS bağlantılarında `?v=N` versiyon query'si var. **Frontend asset'lerini değiştirince `?v=N`'i artır.**
