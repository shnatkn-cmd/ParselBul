...
- **Arayüz:** parsel.ink benzeri **tam ekran harita uygulaması**, **dark theme** (Leaflet, CDN'den).
  - **Harita stili** (sağ üst popup): Uydu (Esri), Sokaklar (OSM), Açık (CARTO Positron), Arazi (Esri Topo).
  - **Arama paneli (sol, sekmeli):** "Adres" (İl/İlçe/Mahalle + Cadde/Sokak + Bina No → geocode) ve "Ada/Parsel" (İl/İlçe/Mahalle + Ada + Parsel).
  ...
  - Sağ bilgi paneli: ada/parsel/mahalle/alan/nitelik/pafta + ★ favori + Google Maps & WhatsApp paylaş.
  - **Ölçüm aracı** (sol alt): mesafe (km/m) ve alan (m²/dönüm, jeodezik).
  - Tüm frontend `public/` (index.html, css/, js/).
...
- **Veritabanı:** Hostinger MySQL — **BAĞLI**. Sunucu: `srv1764.hstgr.io`, DB/kullanıcı: `u851420727_ParselBul`.
  ...
  - **İmar araştırma sonucu:** Türkiye'de tek ücretsiz açık imar API'si yok; parçalı (ulusal e-Plan `e-plan.gov.tr` +
    belediye e-imar sistemleri). Bu yüzden panelde `turkiye.gov.tr/e-imar` derin bağlantısı verildi.
...
- Hostinger eski JS/CSS'i önbellekte tutabiliyor → deploy sonrası "stale asset" hatası (ör. eski main.js'in
  ...
  - `index.html` CSS/JS bağlantılarında `?v=N` versiyon query'si var. **Frontend asset'lerini değiştirince `?v=N`'i artır.** (Güncel: v=5)
